...
// The .md and .json above are for humans and for diffing in review. Code imports these
// instead: `import.meta.dirname` is undefined once a bundler has processed the module,
// so anything that reads the catalogue from disk works in tests and throws in the app.
writeFileSync(
  resolve(out, 'catalogue.ts'),
  `// GENERATED by packages/spec/src/generate.ts — do not edit.\nexport const COMPONENT_CATALOGUE = ${JSON.stringify(catalogue)} as const\n`,
)
writeFileSync(
  resolve(out, 'schema.ts'),
  `// GENERATED by packages/spec/src/generate.ts — do not edit.\nexport const WEBSITE_SPEC_JSON_SCHEMA = ${JSON.stringify(jsonSchema, null, 2)} as const\n`,
)
